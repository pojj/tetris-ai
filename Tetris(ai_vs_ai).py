...
while True:
    ...
    for event in pygame.event.get():
        if event.type==pygame.KEYDOWN:
            ...
            if event.key==pygame.K_RIGHT and game.state=="start" and game2.state=="start":
                game2.move_side(1)
            if event.key==pygame.K_SPACE and game.state=="start" and game2.state=="start":
                game2.hard_drop(pie)
                pie.append(random.randint(0,6))
            if event.key==pygame.K_c and game.state=="start" and game2.state=="start":
    ...
    if refresh%(50-refresh_rate//2+1)==0:
        for i in Tetris_ai.ai(game2.field, game2.figure.shapes[game2.figure.type],game2.figure.x,game2.figure.y,game2.hold,shape.shapes[game2.next]):
            if game.state=="start" and game2.state=="start":
    ...
    if refresh%(50-refresh_rate2//2+1)==0:
        for i in Tetris_ai.ai(game.field, game.figure.shapes[game.figure.type],game.figure.x,game.figure.y,game.hold,shape.shapes[game.next]):
            if game.state=="start" and game2.state=="start":
                if i.key==pygame.K_UP:
                    game.press_up()
                if i.key==pygame.K_LEFT:
    ...
    if (game.score-(game.score%4))>cpu_lines_sent:
        game2.add_garbage()
        cpu_lines_sent+=4
        
    if (game2.score-(game2.score%4))>player_lines_sent:
        game.add_garbage()
        player_lines_sent+=4
